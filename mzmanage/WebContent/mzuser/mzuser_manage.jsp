			...
			<div class="main-content">
				...
				<div class="page-content">
					<div class="page-header">
						<form id="auserSearchForm" name="muserSearchForm"
							action="<spring:url value='/mumanage.do' htmlEscape='true'/>"
								...
								<div class="col-xs-12">
									<div class="table-responsive">
										<table id="sample-table-1"
											...
											<tbody>
												<c:forEach items="${mzUserList}" var="mzUser"
													varStatus="status">
													<tr>
														<td>${mzUser.uid}</td>
														<td>${mzUser['username']}</td>
														<td>${mzUser.nickname}</td>
														<td>${mzUser.groupName}</td>
														<td>${mzUser.regdate}</td>
														<td>
															<c:if test="${mzUser.status == 1}">有效</c:if>
															<c:if test="${mzUser.status == 9}">失效</c:if>
														</td>
														<td>
															<a data-toggle="modal" href="#auserEdit" 
																onClick="editAuser('${mzUser.uid}');" class="btn btn-xs btn-primary"><i class="icon-edit"></i></a>
															<a data-toggle="modal" href="#auserDel"  onClick="delAuser('${mzUser.uid}','${mzUser.username}');" class="btn btn-xs btn-danger"><i class="icon-trash"></i></a>
														</td>
													</tr>
												</c:forEach>
